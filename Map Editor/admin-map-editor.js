  ...
  imageSets: Object.entries(bundledIconFiles.reduce((groups, fileName) => {
    ...
    .map(([name, fileNames]) => ({
      ...
      assets: fileNames.map((fileName) => {
        ...
        return { file: { name: fileName }, image, anchorX: 0.5, anchorY: 1 };
      }),
    })),
  contextLayerId: null,
  placingObject: false,
  poiTypes: [
    { id: 'vila', name: 'Vila', color: '#f0c674', fontSize: 15 },
    { id: 'cidade', name: 'Cidade', color: '#7fdbff', fontSize: 15 },
    { id: 'outro', name: 'Outro', color: '#f1f3ef', fontSize: 15 },
  ],
  maskEditing: null,
...
function createLayer(type = 'terrain') {
  ...
  return {
    ...
    type,
    name: type === 'image' ? `Imagem ${number}` : type === 'object' ? `Objeto ${number}` : type === 'decoration' ? `Decoração ${number}` : type === 'region' ? `Região ${number}` : type === 'path' ? `Caminho ${number}` : type === 'folder' ? `Folder ${number}` : type === 'ground' ? 'Ground / base' : (number === 1 ? 'Cobertura vegetal' : `Terreno ${number}`),
    parentId: null,
    collapsed: false,
    visible: true,
    mask: null,
...
function redraw(includeObjects = true, showSelection = true, includePaths = true) {
  ...
  depthEntries.sort((first, second) => {
    if (state.objectsAlwaysOnTop && Boolean(first.object) !== Boolean(second.object)) return first.object ? 1 : -1;
    return first.y - second.y || second.layerIndex - first.layerIndex;
  });
  for (const entry of depthEntries) {
    if (entry.object) drawMapObject(entry.object, entry.layer);
    else if (entry.decoration) drawDecorationPlacement(entry.layer, entry.decoration);
  ...
  if (showSelection && decorationLayer?.type === 'decoration' && decorationLayer.decoration?.preview) drawDecorationPlacement(decorationLayer, decorationLayer.decoration.preview, ctx, .42);
...
function drawTerrainPlacement(layer, placement, context) {
  ...
  context.drawImage(asset, -width * (assetEntry?.anchorX ?? 0.5), -height * (assetEntry?.anchorY ?? 1), width, height);
  context.restore();
}
...
function drawMapObject(object, layer = null) {
  ...
  ctx.font = `600 ${Math.max(6, Number(type?.fontSize) || 15)}px DM Sans, sans-serif`;
...
function renderLayers() {
  ...
  for (const layer of state.layers) {
    ...
    const layerMeta = layer.type === 'image' ? ['▧', 'Imagem'] : layer.type === 'object' ? ['⌖', 'Objeto'] : layer.type === 'decoration' ? ['✦', 'Decoração'] : layer.type === 'region' ? ['◒', 'Região'] : layer.type === 'path' ? ['〰', 'Caminho'] : layer.type === 'folder' ? ['▰', 'Folder'] : layer.type === 'ground' ? ['▰', 'Ground / base'] : ['⌁', 'Terreno'];
...
function drawDecorationPlacement(layer, placement, context = ctx, alpha = 1) {
  ...
  context.save(); context.globalAlpha = alpha; context.translate(placement.x + (layer.settings.layerOffsetX || 0), placement.y + (layer.settings.layerOffsetY || 0));
...
function renderDecorationCollections(layer) {
  ...
  $('#editDecorationCollection').disabled = !collection; $('#deleteDecorationCollection').disabled = !collection; $('#editDecorationCollection').classList.toggle('active', editing);
}
...
function renderTerrainAnchorPreview(layer) {
  ...
  marker.hidden = !asset;
  positionAnchorMarker(preview, image, marker, asset?.anchorX ?? 0.5, asset?.anchorY ?? 1);
  $('#terrainAnchorCoordinates').textContent = asset ? anchorCoordinateLabel(asset.image, asset.anchorX ?? 0.5, asset.anchorY ?? 1) : `${anchorWord()}: —`;
}
...
function selectLayer(id) {
  ...
  $('#typeBadge').textContent = layer.type === 'image' ? 'Imagem' : layer.type === 'object' ? 'Objeto' : layer.type === 'decoration' ? 'Decoração' : layer.type === 'region' ? 'Região' : layer.type === 'path' ? 'Caminho' : layer.type === 'folder' ? 'Folder' : layer.type === 'ground' ? 'Ground / base' : 'Terreno';
  document.querySelectorAll('.terrain-control, .ground-control, .image-control, .object-control, .decoration-control, .region-control, .path-control').forEach((control) => {
  ...
  if (layer.type === 'decoration') renderDecorationInspector(layer);
...
function renderPoiTypes() {
  ...
  list.replaceChildren(...state.poiTypes.map((type) => {
    ...
    const fontSize = document.createElement('input'); fontSize.type = 'number'; fontSize.min = '6'; fontSize.max = '72'; fontSize.value = type.fontSize || 15; fontSize.title = 'Tamanho da fonte no mapa'; fontSize.ariaLabel = 'Tamanho da fonte no mapa';
    ...
    fontSize.oninput = () => { type.fontSize = Math.max(6, Math.min(72, Number(fontSize.value) || 15)); redraw(); };
    remove.onclick = () => {
    ...
    item.append(name, color, fontSize, remove); return item;
  }));
}
...
$('#createPoiType').onclick = () => {
  ...
  state.poiTypes.push({ id, name, color: $('#newPoiTypeColor').value, fontSize: 15 });
...
$('#saveDetailedDescription').onclick = () => {
  ...
  layer.object.descriptionPages = editingDescriptionPages.map((page) => ({ id: crypto.randomUUID?.() || `${Date.now()}-${Math.random()}`, title: page.title, content: editingTemplateStructureOnly ? '' : page.content, images: editingTemplateStructureOnly ? [] : [...page.images], autoOverview: Boolean(page.autoOverview) }));
  updateDetailedDescriptionControls(layer.object);
  ...
  await new Promise((resolve) => {
    function drawBatch() {
      ...
      for (; drawn < batchEnd; drawn++) {
        ...
        outputContext.drawImage(placement.asset, -width * (assetEntry?.anchorX ?? 0.5), -height * (assetEntry?.anchorY ?? 1), width, height);
...
$('#selectAllDecorationCollection').onclick = () => { const layer = selectedLayer(); if (layer?.type !== 'decoration') return; const collection = state.decorationCollections.find((item) => item.id === layer.decoration.collectionId); layer.decoration.selectedAssetIds = collection ? [...collection.assetIds] : projectSpriteEntries().map((entry) => entry.id); renderDecorationInspector(layer); redraw(); };
...
document.querySelectorAll('[data-layer-type]').forEach((button) => {
  button.onclick = () => {
    ...
    if (activeFolder) layer.parentId = activeFolder.id;
...
function openMapSettings() {
  ...
  $('#maskChunkSizeSetting').value = state.maskChunkSize; $('#debugChunkGridSetting').checked = state.debugChunkGrid; $('#objectsAlwaysOnTopSetting').checked = state.objectsAlwaysOnTop;
...
$('#applyMapSettings').onclick = () => {
  ...
  state.objectsAlwaysOnTop = $('#objectsAlwaysOnTopSetting').checked;
...
function renderImageSets() {
  ...
  for (const set of visibleSets) {
    ...
    item.innerHTML = '<img class="set-thumbnail" alt=""><div><b></b><small></small></div><button class="edit-set" title="Editar">✎</button><button class="delete-set" title="Excluir conjunto">×</button><button class="use-set">Usar</button>';
    ...
    item.querySelector('.delete-set').onclick = () => {
      if (!window.confirm(`Excluir o conjunto de imagens “${set.name}”?`)) return;
      state.imageSets = state.imageSets.filter((item) => item.id !== set.id);
      state.layers.forEach((layer) => {
        if (layer.object?.iconSetId === set.id) { layer.object.iconSetId = ''; layer.object.selectedIconIndex = 0; }
        if (layer.object?.gallerySetId === set.id) layer.object.gallerySetId = '';
        if (layer.path?.gallerySetId === set.id) layer.path.gallerySetId = '';
        if (layer.decoration) { layer.decoration.selectedAssetIds = layer.decoration.selectedAssetIds.filter((id) => !id.startsWith(`${set.id}:`)); layer.decoration.placements = layer.decoration.placements.filter((placement) => !placement.assetId.startsWith(`${set.id}:`)); }
      });
      state.decorationCollections.forEach((collection) => { collection.assetIds = collection.assetIds.filter((id) => !id.startsWith(`${set.id}:`)); });
      if (editingSetId === set.id) { editingSetId = null; pendingSetAssets = []; $('#setName').value = ''; renderEditingSetAssets(); }
      populateObjectOptions(); renderImageSets(); redraw();
    };
  ...
  try {
    ...
    const importedSets = await Promise.all(packageData.sets.map(async (set) => {
      ...
      return {
        ...
        assets: await Promise.all(set.assets.map(async (asset) => ({ file: { name: String(asset.name || 'imagem.png') }, image: await imageFromSource(asset.source), anchorX: asset.anchorX ?? 0.5, anchorY: asset.anchorY ?? 1 }))),
      };
    }));
...
function descriptionAsHtml(value) {
  const element = document.createElement('div'); element.textContent = String(value ?? '');
  return `<p>${element.innerHTML}</p>`;
}
function syncOverviewDescriptionPage(entity, value) {
  if (!entity) return;
  entity.description = String(value ?? '');
  entity.descriptionPages ||= [];
  const index = entity.descriptionPages.findIndex((page) => page.autoOverview);
  if (!entity.description.trim()) {
    if (index >= 0) entity.descriptionPages.splice(index, 1);
    return;
  }
  const overview = { ...(index >= 0 ? entity.descriptionPages[index] : {}), id: index >= 0 ? entity.descriptionPages[index].id : (crypto.randomUUID?.() || `overview-${Date.now()}`), title: 'Visão Geral', content: descriptionAsHtml(entity.description), images: [], autoOverview: true };
  if (index >= 0) entity.descriptionPages[index] = overview;
  else entity.descriptionPages.unshift(overview);
}
function descriptionPagesWithOverview(entity) {
  const pages = [...(entity?.descriptionPages || [])];
  const description = String(entity?.description ?? entity?.descricao ?? '').trim();
  if (description && !pages.some((page) => page.autoOverview)) pages.unshift({ id: 'overview', title: 'Visão Geral', content: descriptionAsHtml(description), images: [], autoOverview: true });
  return pages;
}
...
for (const [elementId, property] of Object.entries(objectBindings)) {
  $(`#${elementId}`).addEventListener(elementId === 'objectPoi' ? 'change' : 'input', (event) => {
    ...
    if (property === 'description') syncOverviewDescriptionPage(layer.object, event.target.value);
...
for (const [elementId, property] of Object.entries(pathBindings)) {
  $(`#${elementId}`).addEventListener(elementId === 'pathShowOnMap' ? 'change' : 'input', (event) => {
    ...
    if (property === 'description') syncOverviewDescriptionPage(layer.path, event.target.value);
...
$('#layerContextMenu').addEventListener('click', (event) => {
  ...
  if (action === 'rename') {
  ...
  } else if (action === 'folderize') {
    const name = window.prompt('Nome da nova pasta:', 'Nova pasta');
    if (name?.trim()) { const folder = createLayer('folder'); folder.name = name.trim(); folder.parentId = layer.parentId || null; const index = state.layers.indexOf(layer); state.layers.splice(index, 0, folder); layer.parentId = folder.id; folder.collapsed = false; normalizeLayerTreeOrder(); }
    ...
    if (layer.decoration) duplicate.decoration = JSON.parse(JSON.stringify(layer.decoration));
...
function serializeAsset(asset) {
  return { name: asset.file.name, source: asset.image.src, anchorX: asset.anchorX, anchorY: asset.anchorY ?? 1 };
}

function serializedChunks(chunks) { return chunks instanceof Map ? [...chunks.values()] : []; }

const PROJECT_IMAGE_LIMIT = 500 * 1024;
async function jpegProjectImage(source, cache) {
  ...
  let quality = .9, result = null;
  while (!result || result.size > PROJECT_IMAGE_LIMIT) {
    const surface = document.createElement('canvas'); surface.width = Math.max(1, Math.round(width)); surface.height = Math.max(1, Math.round(height));
    ...
    result = await new Promise((resolve, reject) => surface.toBlob((blob) => blob ? resolve(blob) : reject(new Error('Não foi possível comprimir uma imagem do projeto')), 'image/jpeg', quality));
    if (result.size <= PROJECT_IMAGE_LIMIT) break;
    if (quality > .35) quality -= .1;
    else { width *= .82; height *= .82; quality = .82; }
    await nextFrame();
  }
...
function createProjectData() {
  ...
  return {
    ...
    layers: state.layers.map((layer) => ({
      ...
      assets: layer.assets.map(serializeAsset), settings: layer.settings, object: layer.object, decoration: layer.decoration, region: layer.region, path: layer.path,
...
$('#projectInput').addEventListener('change', async (event) => {
  ...
  try {
    ...
    state.debugChunkGrid = Boolean(project.debugChunkGrid); state.objectsAlwaysOnTop = Boolean(project.objectsAlwaysOnTop); updateChunkDebugGrid();
    $('#mapFilterSetting').value = state.mapFilter; applyMapFilter();
    state.imageSets = await Promise.all(project.imageSets.map(async (set) => ({
      ...set, assets: await Promise.all(set.assets.map(async (asset) => ({ file: { name: asset.name }, image: await imageFromSource(asset.source), anchorX: asset.anchorX, anchorY: asset.anchorY ?? 1 }))),
    })));
    state.layers = await Promise.all(project.layers.map(async (saved) => {
      ...
      if (saved.decoration) layer.decoration = { ...layer.decoration, ...saved.decoration, preview: null };
      ...
      layer.assets = await Promise.all(saved.assets.map(async (asset) => ({ file: { name: asset.name }, image: await imageFromSource(asset.source), anchorX: asset.anchorX, anchorY: asset.anchorY ?? 1 })));
...
function layerAtPoint(point) {
  for (const layer of state.layers) {
    ...
    if (layer.type === 'decoration' && layer.decoration?.placements?.some((placement) => {
      const asset = decorationAsset(placement.assetId), size = 48 * (placement.size || 1), width = size * ((asset?.image.naturalWidth || 48) / (asset?.image.naturalHeight || 48));
      return point.x >= placement.x + offsetX - width * (asset?.anchorX ?? .5) && point.x <= placement.x + offsetX + width * (1 - (asset?.anchorX ?? .5)) && point.y >= placement.y + offsetY - size * (asset?.anchorY ?? 1) && point.y <= placement.y + offsetY + size * (1 - (asset?.anchorY ?? 1));
    })) return layer;
...
function exportablePois() {
  return state.layers.filter((layer) => layer.visible && layer.type === 'object' && layer.object?.poi && layer.object.x !== null).map((layer) => {
    ...
    return { ...object, descriptionPages: descriptionPagesWithOverview(object), description: String(object.description ?? object.descricao ?? ''), simpleDescription: String(object.description ?? object.descricao ?? ''), x: object.x + (object.offsetX ?? 0) + (layer.settings.layerOffsetX || 0), y: object.y + (object.offsetY ?? 0) + (layer.settings.layerOffsetY || 0), id: layer.id, icon: objectIconAsset(object)?.image.src || '', iconNaturalWidth: objectIconAsset(object)?.image.naturalWidth || 48, iconNaturalHeight: objectIconAsset(object)?.image.naturalHeight || 48, gallery: gallerySet?.assets.map((asset) => asset.image.src) || [], typeName: type?.name || object.type, color: type?.color || '#fff', fontSize: type?.fontSize || 15 };
  }).filter((poi) => poi.icon);
}
...
function exportablePaths() {
  return state.layers.filter((layer) => layer.visible && layer.type === 'path' && layer.path?.showOnMap && layer.path.points.length > 1).map((layer) => {
    ...
    return { id: layer.id, name: layer.path.name, description: String(layer.path.description ?? layer.path.descricao ?? ''), descriptionPages: descriptionPagesWithOverview(layer.path), points: layer.path.points.map((point) => ({ x: point.x + (layer.settings.layerOffsetX || 0), y: point.y + (layer.settings.layerOffsetY || 0) })), distance: layer.path.distance || calculatePathDistance(layer.path.points), gallery, preset };
  });
}

function syncActiveDescriptionField() {
  const layer = selectedLayer();
  if (layer?.type === 'object') syncOverviewDescriptionPage(layer.object, $('#objectDescription').value);
  if (layer?.type === 'path') syncOverviewDescriptionPage(layer.path, $('#pathDescription').value);
}

function createMapHtml() {
  // Capture the currently focused textarea even if the browser has not yet
  // dispatched its final input/change event before Preview or Export.
  syncActiveDescriptionField();
