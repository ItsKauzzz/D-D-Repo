...
const phraseTranslations = {
  ...
  ja: {
    ...
    'Nome do local': '場所の名前', 'História, detalhes e informações do local...': '場所の歴史、詳細、情報...', 'Ex.: Fronteiras políticas': '例：政治的国境', 'Ex.: Reino do Norte': '例：北の王国', 'Ex.: Árvores de pinheiro': '例：松の木', 'Âncora': 'アンカー', 'Caminhos': '道', 'Caminho': '道', 'Nome do caminho': '道の名前', 'Descrição do caminho...': '道の説明...', 'Mostrar no mapa': 'マップに表示', 'Preset visual': '表示プリセット', 'Nome do preset': 'プリセット名', 'Stroke': '線幅', 'Tracejado / pontilhado': '破線 / 点線', 'Salvar como preset': 'プリセットとして保存', '〰 Desenhar caminho': '〰 道を描く', 'Distância: 0 km': '距離: 0 km', 'Folder': 'フォルダー', 'PROJETO': 'プロジェクト', 'Configurações': '設定', 'Escala do mapa': 'マップ縮尺', 'equivalem a': '相当',
  },
};
...
function renderLayers() {
  ...
  for (const folder of state.layers.filter((layer) => layer.type === 'folder')) {
    ...
    const wrapper = document.createElement('section'); wrapper.className = 'folder-layer';
...
$('#projectSettingsBtn').onclick = () => { $('#distanceScaleKm').value = state.distanceScaleKm; $('#projectSettingsModal').hidden = false; };
...
function createProjectData() {
  return {
    ...
    layers: state.layers.map((layer) => ({
      id: layer.id, type: layer.type, name: layer.name, visible: layer.visible, parentId: layer.parentId, selectedAssetIndex: layer.selectedAssetIndex,
...
$('#projectInput').addEventListener('change', async (event) => {
  ...
  try {
    ...
    state.layers = await Promise.all(project.layers.map(async (saved) => {
      ...
      Object.assign(layer, { id: saved.id, name: saved.name, visible: saved.visible, parentId: saved.parentId || null, maskName: saved.maskName, selectedAssetIndex: saved.selectedAssetIndex || 0, settings: { ...layer.settings, ...saved.settings } });
