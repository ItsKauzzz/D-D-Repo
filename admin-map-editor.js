...
const state = {
  ...
  regionPresets: [{ id: 'regions', name: 'Regiões', color: '#6fa86b', defaultOverview: false }],
  drawingPath: false,
  distanceScaleKm: 100,
};
...
function createLayer(type = 'terrain') {
  ...
  return {
    ...
    region: type === 'region' ? { group: 'Regiões', presetId: 'regions', name: `Região ${number}`, color: '#6fa86b', defaultOverview: false, drawnAt: 0 } : null,
...
$('#regionPreset').addEventListener('change', async (event) => {
  ...
  if (event.target.value !== '__new__') {
    ...
    Object.assign(layer.region, { presetId: preset.id, group: preset.name, color: preset.color, defaultOverview: preset.defaultOverview });
...
$('#saveRegionPreset').onclick = async () => {
  ...
  const values = { name: $('#regionPresetName').value.trim() || `Tipo ${state.regionPresets.length + 1}`, color: $('#regionColor').value, defaultOverview: $('#regionDefaultOverview').checked };
  ...
  const layer = selectedLayer();
  state.layers.filter((item) => item.type === 'region' && (item === layer || item.region.presetId === preset.id)).forEach((item) => Object.assign(item.region, { presetId: preset.id, group: preset.name, color: preset.color, defaultOverview: preset.defaultOverview }));
  if (layer.mask) await applyRegionPriority(layer);
...
$('#deleteRegionPreset').onclick = () => {
  ...
  if (!state.regionPresets.length) state.regionPresets.push({ id: 'regions', name: 'Regiões', color: '#6fa86b', defaultOverview: false });
  const fallback = state.regionPresets[0];
  state.layers.filter((layer) => layer.type === 'region' && layer.region.presetId === id).forEach((layer) => Object.assign(layer.region, { presetId: fallback.id, group: fallback.name, color: fallback.color, defaultOverview: fallback.defaultOverview }));
...
function exportableRegions() {
  return state.layers.filter((layer) => layer.visible && layer.type === 'region' && layer.mask).map((layer) => {
    ...
    return { id: layer.id, group: layer.region.group || 'Regiões', name: layer.region.name, color: layer.region.color, defaultOverview: Boolean(layer.region.defaultOverview), mask: layer.mask.src, centerX: count ? totalX / count : canvas.width / 2, centerY: count ? totalY / count : canvas.height / 2 };
  });
}
